...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('External AML export error:', error)
      return NextResponse.json(
        { error: 'Failed to fetch AML checks' },
        { status: 500 },
      )
    }
    ...
    await logAuditEvent({
      ...
      lawful_basis: 'Legal obligation (AML support) / BI feed',
    })
    ...
    console.error('External AML export unexpected error:', err)
    return NextResponse.json(
