...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('External audit-events export error:', error)
    ...
    await logAuditEvent({
      ...
      lawful_basis: 'GDPR accountability / AML audit support',
    })
    ...
    console.error('External audit-events export unexpected error:', err)
    return NextResponse.json(
