// app/api/external/export/clients/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = createSupabaseServerClientStrict()
      .from('clients')
      .select('*')
      .eq('firm_id', firmId)
    ...
    if (error) {
      console.error('External clients export error:', error)
    ...
    console.error('External clients export unexpected error:', err)
    return NextResponse.json(
