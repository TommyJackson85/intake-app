...
export async function GET(request: NextRequest) {
  try {
    // Get API key and firm/scopes
    const apiKey = request.headers.get('x-firm-api-key');
    const { firm_id: firmId, scopes } = await getFirmFromApiKeyWithScopes(apiKey);
    ...
    const resolvedFirmId = profile.firm_id as string;
    ...
    try {
      // ✅ Use positional arguments expected by logAuditEvent
      await logAuditEvent(
        'GDPR_EXPORT_REQUESTED',                  // eventKey / type
        'User GDPR export generated',             // description
        ip,                                       // ip
        user.id,                                  // userId (string | undefined)
        resolvedFirmId,                           // firmId (string | undefined)
        {
          entity_type: 'profile',
          entity_id: user.id,
          scope: 'user_only',
          route: '/api/gdpr/export/me',
          method: 'GET',
        }
      );
    } catch (auditError2) {
