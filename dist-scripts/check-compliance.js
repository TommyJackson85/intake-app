...
function loadDataRetentionSchedule() {
...
function checkRetentionVsSchedule(retentionCfg, scheduleMd) {
    const missingInSchedule = [];
    for (const tableOrCategory of Object.keys(retentionCfg)) {
        if (tableOrCategory.startsWith('_'))
            continue; // meta entries like _backups
    ...
    if (missingInSchedule.length > 0) {
        fail(`These entries exist in retention.config.json but are not mentioned in DATA_RETENTION_SCHEDULE.md: ${missingInSchedule.join(', ')}`);
    }
}
// 2) Ensure important schema tables have retention entries
function checkSchemaVsRetention(retentionCfg, schemaTables) {
    ...
    if (missing.length > 0) {
        fail(`These schema tables are present in supabase/migrations but missing in retention.config.json: ${missing.join(', ')}`);
    }
}
...
async function checkAmlAccessLogging() {
    ...
    for (const file of apiFiles) {
        ...
        if (code.includes("from('aml_checks'") ||
            code.includes('from("aml_checks"')) {
            const hasAuditImport = code.includes("from '@/lib/auditLog'") ||
    ...
    if (offenders.length > 0) {
        fail(`The following API files query aml_checks but do not appear to use logAuditEvent/audit logging: ${offenders.join(', ')}`);
    }
}
async function main() {
