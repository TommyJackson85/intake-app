...
export default function MatterDetailModal({ matter, open, onClose, onArchive, initialTab }: MatterDetailModalProps) {
  const {
    ...
    getCondoDiligence,
    patchCondoDiligence,
    ensurePostClosingUndertakings,
    getPostClosingUndertakings,
    patchPostClosingUndertakings,
  } = useDemoStore()
  ...
  const postClosingSaveLockRef = useRef(false)
  ...
  useEffect(() => {
    if (!open || !effectiveMatter) return
    ensurePostClosingUndertakings(effectiveMatter.id)
  }, [open, effectiveMatter, ensurePostClosingUndertakings])
  ...
  const fincenBeneficialOwnershipDashboard = useMemo(() => {
    if (!effectiveMatter) return null
    return buildFinCENBeneficialOwnershipReviewDashboard(effectiveMatter)
  }, [effectiveMatter])
        ...
        <div style={{ padding: '16px 18px', overflowY: 'auto', flex: 1 }}>
          {activeTab === 'Overview' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {(showCondoDiligenceTab ||
                ...
                fincenBeneficialOwnershipDashboard ||
              ...
              {fincenBeneficialOwnershipDashboard && (
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 900, color: '#134252' }}>
                        FinCEN Beneficial Ownership Review
                      </div>
                      <div style={{ fontSize: 11, color: '#627c71', marginTop: 4, lineHeight: 1.45, maxWidth: '40rem' }}>
                        {fincenBeneficialOwnershipDashboard.disclaimer}
                      </div>
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                      <span
                        style={{
                          ...
                          fontSize: 11,
                          fontWeight: 900,
                          background: fincenBeneficialOwnershipDashboard.workspaceStatus.bg,
                          color: fincenBeneficialOwnershipDashboard.workspaceStatus.color,
                          border: `1px solid ${fincenBeneficialOwnershipDashboard.workspaceStatus.border}`,
                          whiteSpace: 'nowrap',
                        }}
                      >
                        {fincenBeneficialOwnershipDashboard.workspaceStatus.label}
                      </span>
                      <button
                        type="button"
                        onClick={() => goToFinCENSection(FINCEN_SECTION_IDS.beneficialOwners)}
                        aria-label="Open FinCEN and AML beneficial ownership section from review"
                        style={{
                          background: 'white',
                          border: '1px solid rgba(94,82,64,0.25)',
                          borderRadius: 6,
                  ...
                  >
                    <div style={{ border: '1px solid rgba(94,82,64,0.1)', borderRadius: 8, padding: '8px 10px', background: '#fcfcf9' }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: '#627c71' }}>Demo gate</div>
                      <div style={{ fontSize: 12, fontWeight: 800, color: '#134252', marginTop: 4 }}>
                        {fincenBeneficialOwnershipDashboard.eligibleUnderDemoGate ? 'Open' : 'Outside gate'}
                      </div>
                    </div>
                    <div style={{ border: '1px solid rgba(94,82,64,0.1)', borderRadius: 8, padding: '8px 10px', background: '#fcfcf9' }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: '#627c71' }}>Owners</div>
                      <div style={{ fontSize: 12, fontWeight: 800, color: '#134252', marginTop: 4 }}>
                        {fincenBeneficialOwnershipDashboard.ownerCount}
                      </div>
                    </div>
                    <div style={{ border: '1px solid rgba(94,82,64,0.1)', borderRadius: 8, padding: '8px 10px', background: '#fcfcf9' }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: '#627c71' }}>Certified</div>
                      <div style={{ fontSize: 12, fontWeight: 800, color: '#134252', marginTop: 4 }}>
                        {fincenBeneficialOwnershipDashboard.certifiedOwnerCount}
                      </div>
                    </div>
                    <div style={{ border: '1px solid rgba(94,82,64,0.1)', borderRadius: 8, padding: '8px 10px', background: '#fcfcf9' }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: '#627c71' }}>Attention rows</div>
                      <div style={{ fontSize: 12, fontWeight: 800, color: '#134252', marginTop: 4 }}>
                        {fincenBeneficialOwnershipDashboard.attentionRowCount}
                      </div>
                    </div>
                  </div>

                  <div style={{ fontSize: 12, color: '#134252', lineHeight: 1.45 }}>
                    <span style={{ fontWeight: 800 }}>Next action:</span> {fincenBeneficialOwnershipDashboard.nextAction}
                  </div>
                  ...
                  >
                    {fincenBeneficialOwnershipDashboard.regulatoryNote}
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {fincenBeneficialOwnershipDashboard.rows.map((row) => (
                      <div
                        key={row.id}
                        style={{
                          display: 'flex',
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const review = createConflictCheckReviewPatch({
                              ...
                              screeningSummary:
                                normalizeConflictCheckReview(linkedConflictIntakeLead.conflictCheckReview)
                                  .screeningSummary ||
                                linkedConflictIntakeLead.conflict_check_note ||
                                conflictScreening?.summary ||
                                null,
                            })
                            patchIntakeLead(linkedConflictIntakeLead.id, { conflictCheckReview: review })
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#627c71', marginTop: 4, lineHeight: 1.45, maxWidth: '42rem' }}>
                        {POST_CLOSING_UNDERTAKINGS_DISCLAIMER}
                      </div>
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                  ...
                  {postClosingError ? (
                    <div style={{ color: '#c0152f', fontSize: 12, fontWeight: 700 }}>{postClosingError}</div>
                  ) : null}

                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                    <button
                      type="button"
                      disabled={postClosingSaveStatus === 'saving'}
                      onClick={() => {
                        if (!effectiveMatter) return
                        if (postClosingSaveLockRef.current || postClosingSaveStatus === 'saving') return
                        postClosingSaveLockRef.current = true
                        setPostClosingSaveStatus('saving')
                        setPostClosingError(null)
                        try {
                          ensurePostClosingUndertakings(effectiveMatter.id)
                          const actor = staff[0]
                          const next = createPostClosingUndertakingsReviewPatch({
                            ...
                            actor: {
                              ...
                              staffName: actor?.full_name || 'Staff',
                            },
                            existing: getPostClosingUndertakings(effectiveMatter.id),
                          })
                          patchPostClosingUndertakings(effectiveMatter.id, next)
                          setPostClosingSaveStatus('saved')
                          window.setTimeout(() => {
                            setPostClosingSaveStatus('idle')
                      ...
                      style={{
                        ...
                        borderRadius: 6,
                        border: '1px solid #134252',
                        background: postClosingSaveStatus === 'saving' ? '#f0f0f0' : '#134252',
                        color: postClosingSaveStatus === 'saving' ? '#627c71' : '#fff',
                        fontWeight: 800,
                        fontSize: 12,
                        cursor: postClosingSaveStatus === 'saving' ? 'not-allowed' : 'pointer',
                      }}
                    >
                      {postClosingSaveStatus === 'saving'
                        ? 'Saving…'
                        : postClosingSaveStatus === 'saved'
                          ? 'Saved'
          ...
          {activeTab === 'Documents' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {(linkedConflictIntakeLead || conflictMemoHistory.length > 0) && (
                <div
                  style={{
                    border: '1px solid rgba(94,82,64,0.12)',
                    borderRadius: 8,
                ...
                >
                  ...
                  {conflictMemoHistory.length === 0 ? (
                    <div style={{ color: '#627c71', fontSize: 13 }}>
                      No saved internal conflict check memos yet. Generate and save one from Overview → Conflict Check
                      Review.
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
